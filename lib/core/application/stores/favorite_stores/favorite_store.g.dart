// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'favorite_store.dart';

// **************************************************************************
// StoreGenerator
// **************************************************************************

// ignore_for_file: non_constant_identifier_names, unnecessary_brace_in_string_interps, unnecessary_lambdas, prefer_expression_function_bodies, lines_longer_than_80_chars, avoid_as, avoid_annotating_with_dynamic, no_leading_underscores_for_local_identifiers

mixin _$FavoriteStores on FavoriteStoresBase, Store {
  late final _$favoritesAtom = Atom(
    name: 'FavoriteStoresBase.favorites',
    context: context,
  );

  @override
  ObservableList<Character> get favorites {
    _$favoritesAtom.reportRead();
    return super.favorites;
  }

  @override
  set favorites(ObservableList<Character> value) {
    _$favoritesAtom.reportWrite(value, super.favorites, () {
      super.favorites = value;
    });
  }

  late final _$loadFavoritesAsyncAction = AsyncAction(
    'FavoriteStoresBase.loadFavorites',
    context: context,
  );

  @override
  Future<void> loadFavorites() {
    return _$loadFavoritesAsyncAction.run(() => super.loadFavorites());
  }

  late final _$addFavoriteAsyncAction = AsyncAction(
    'FavoriteStoresBase.addFavorite',
    context: context,
  );

  @override
  Future<void> addFavorite(Character character) {
    return _$addFavoriteAsyncAction.run(() => super.addFavorite(character));
  }

  late final _$removeFavoriteAsyncAction = AsyncAction(
    'FavoriteStoresBase.removeFavorite',
    context: context,
  );

  @override
  Future<void> removeFavorite(Character character) {
    return _$removeFavoriteAsyncAction.run(
      () => super.removeFavorite(character),
    );
  }

  late final _$toggleFavoriteAsyncAction = AsyncAction(
    'FavoriteStoresBase.toggleFavorite',
    context: context,
  );

  @override
  Future<void> toggleFavorite(Character character) {
    return _$toggleFavoriteAsyncAction.run(
      () => super.toggleFavorite(character),
    );
  }

  @override
  String toString() {
    return '''
favorites: ${favorites}
    ''';
  }
}
